...
-- Create tables with NO primary keys or indexes initially
CREATE TABLE users (
    id BIGINT,
    ...
    -- Intentionally NO primary key or indexes
    status VARCHAR(50)
);
...
CREATE TABLE orders (
    ...
    status VARCHAR(50)
    -- Intentionally NO primary key, indexes, or foreign keys
);
...
BEGIN
    ...
    -- Insert 1,000,000 orders (this will be slow by design)
    SET i = 1;
    WHILE i <= 1000000 DO
        INSERT INTO orders (id, user_id, product_id, quantity, total_amount, order_date, status)
        VALUES (i,
                1 + FLOOR(RAND() * 100000),  -- Random user_id
                1 + FLOOR(RAND() * 50000),   -- Random product_id
                1 + FLOOR(RAND() * 10),
                ROUND(RAND() * 1000, 2),
